...
export default function HomeWithApiRoute() {
  ...
  const [selectedReplyStyle, setSelectedReplyStyle] =
      ...
      if (DOING_WARMUP) {
        try {
          ...
          if (
            hasWarmedUp &&
            lastWarmup &&
            now - parseInt(lastWarmup) < WARMUP_COOLDOWN
          ) {
            return;
          }
          ...
          throw new Error(
            `Failed to warm up OpenRouter API: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
    };
        ...
        <CardBody className="gap-6 pt-6">
          <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
            <div>
              ...
              >
                <Tab key="reply" className="text-default-600" title="Reply" />
                <Tab key="create" className="text-default-600" title="Create" />
              </Tabs>
            </div>
            ...
            <div>
              ...
              >
                <Tab
                  ...
                  title="Default"
                />
                <Tab key="dad" className="text-default-600" title="Dad" />
                <Tab key="huzz" className="text-default-600" title="Huzz" />
                <Tab
                  key="gravy"
                  className="text-default-600"
                  title="Gravy Seal"
            ...
            <div>
              <Textarea
                ...
                placeholder={
                  mode === "reply"
                    ? "Enter the message you want to reply to..."
                    : "Enter your message..."
                }
                value={message}
                variant="bordered"
                  ...
                  components={{
                    ...
                    code: ({ children, node }) =>
                      ...
                      node.properties.className.some(
                        ...
                        <code className="block bg-default-200 p-2 rounded text-sm font-mono">
                          {children}
                        </code>
                      ) : (
                        <code className="bg-default-200 px-1 py-0.5 rounded text-sm font-mono">
                          {children}
                        </code>
                      ),
                    blockquote: ({ children }) => (
                      <blockquote className="border-l-2 border-default-300 pl-3 italic">
                        {children}
                      </blockquote>
                    ),
                  }}
