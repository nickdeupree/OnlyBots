...
export async function generateReplyBackend(
  ...
  try {
    ...
    const chosenSysPromptAddon =
      sysPromptAddons[randomIndex] + " and " + sysPromptAddons[randomIndexTwo];
    ...
    if (!apiKey) {
      throw new Error(
        "No API key available. Please provide your own OpenRouter API key.",
      );
    }
